...
namespace Microsoft.MixedReality.Toolkit.WindowsMixedReality.Input
{
    ...
    public abstract class BaseWindowsMixedRealitySource : BaseController
    {
        ...
        {
            bool selectPressed = false;// interactionSourceState.selectPressed;
            if (PinchDetectorManager.Instance == null)
            {
                Debug.LogWarning("The <PinchDetectorManager> is missing! Please add the following prefab to your Vertical Slice scene: 'Interaction.Core -> Prefabs -> BondiInputHandlers'!");
                selectPressed = interactionSourceState.selectPressed;
            }
            else
            {
                if (PinchDetectorManager.Instance.hands_PinchMode == PinchDetectorManager.HandPinchModes.System_IsGrasped)
                {
                    selectPressed = interactionSourceState.grasped;
                }
                else if (PinchDetectorManager.Instance.hands_PinchMode == PinchDetectorManager.HandPinchModes.System_SelectPressed)
                {
                    selectPressed = interactionSourceState.selectPressed;
                }
                else if (PinchDetectorManager.Instance.hands_PinchMode == PinchDetectorManager.HandPinchModes.Custom_OverwriteSystem)
                {
                    selectPressed = PinchDetectorManager.Instance.IsPinching;
                }

                PinchDetectorManager.Instance.IsPinching = selectPressed;
            }            
